...
def pass_token():
    ...
    while True:  
        if node_state["has_token"]:
            next_node_url = node_state["node_urls"][node_state["current_index"]]
            print(f"Tentando passar o token para: {next_node_url}")
            time.sleep(2)      
            try:
                response = requests.post(f"{next_node_url}/receive_token", timeout=5)
